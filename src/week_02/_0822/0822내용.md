...
```java
// 현재 root : com.dev/service/common/
package com.dev.service.common;
public class Dice {
...
```java
// 현재 root : com.dev/service/
package com.dev.service;
import com.dev.service.common.Dice;
...
* 클래스 메서드는 `클래스명.메서드명()` 으로 사용가능.
* `메서드 안에 선언된 변수`들은 메서드가 실행될 때 메모리에 생성되었다가, `메서드가 종료`될 때 사라짐.
