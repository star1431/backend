...
```mermaid
sequenceDiagram
    participant C as Client (브라우저 / Postman)
    ...
    alt 토큰 없음
        ...
        JT-->>F: (5) Claims 반환 (userId, name, email, roles)
        F->>CU: (6) CustomUserDetails 생성<br>(userId, name, email, roles)
    ...
    F-->>C: (9) 다음 필터로 요청 전달 (filterChain.doFilter)
```
...
sequenceDiagram
    ...
    alt refreshToken 없음
        ...
        alt DB 토큰 불일치
            ...
            UC-->>C: (14) 200 OK<br>(UserLoginResponseDto : 새 AccessToken)
        end
    end
```
