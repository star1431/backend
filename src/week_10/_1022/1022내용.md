...
### 엔티티의 GeneratedValue 타입 종류

`@GeneratedValue(strategy = GenerationType.XXX)`

| 타입 | 설명 |
|---|---|
| AUTO | 기본 전략, 데이터베이스에 따라 자동 선택 |
| IDENTITY | 데이터베이스의 자동 증가 컬럼 사용 (MySQL의 AUTO_INCREMENT) |
| SEQUENCE | 데이터베이스 시퀀스 사용 (Oracle, PostgreSQL 등) |
| TABLE | 별도의 테이블을 사용하여 키 생성 (비추천) |
